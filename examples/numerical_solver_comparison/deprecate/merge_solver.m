...
% compute the exact solutions at time 
t_sol = 30;
tol = [1e-6; 1e-2]; % dt_tol, dx_tol
M_exact = struct;
for link = net.link_labels'
   
    linkStr = sprintf('link_%d', link);
    M_exact.(linkStr) = Mos.getSolutionAtTime(t_sol, link, tol);
    
end

timing_end = now;

fprintf('Solving the junction took: %f s\n', (timing_mid-timing_start)*86400);
