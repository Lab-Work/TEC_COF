...
classdef postSolution_MPC < handle
    ...
    methods
        ...
        function self=postSolution_MPC(x, net, dv_index, end_time, dx_res, dt_res)
        ...
        function estimateState(self)
            ...
            for link = net.link_labels'
                ...
                self.t_mesh_s = 0:self.dt_res:end_time;
        ...
        function [TF, steps] = checkEntropy(self, entropyTol)
            ...
            for junc = self.net.junc_labels'
                ...
                % The entropic solution should be 
                % q_upFreeway = q_downFreeway_receiving - q_onramp
                elseif strcmp(self.net.network_junc.(juncStr).type_junc,'onrampjunc')
                    ...
                    downFreewayStr = sprintf('link_%d',outlink);
                    ...
                    for i = 1: length(T_grid)
                        ...
                        d_M = self.samplePointsLink(t_end, outlink, 'upstream');
                        
                        if abs(q_up(i)*T_grid(i) - q_on(i)*T_grid(i) - d_M) > entropyTol
                            % difference greater than the tolerance
                            % This may due to the discretization or we
                            % simply did not add entropic component in the
                            % objective function
        ...
        %        dt_tol, float, the minimal resolution we will investigate
        ...
        % output:
        %       M: the vehicle id at the position with M=0 at left bottom
        function M = samplePoint(self, link, times, positions)
            ...
            for i = 1:length(times)
                ...
                % In samplePointsLink(), we remove the closest piece of boundary condition 
        ...
        % input:
        %       link: the link ID
        %       time: 2x1 float, the time of two points
        %       position: 2x1 float, the position of two points in meters
        % output:
        %       pt_found: struct, .time, a column vector
        %                         .position, a column vector, in meters
        %                         .vehicle_id, a column vector, absolute vehicle ID
        function pt_found = searchShockIntersection(self, link, time, position)
        ...
        function density = extractDensity(self, time)
            ...
            for link = self.net.link_labels'
                ...
                pt_found = self.searchShockIntersection(link, [time; time], [0; len_link]);

                % compute the density
                dx = pt_found.position(2:end) - pt_found.position(1:end-1);
                dM = pt_found.vehicle_id(2:end) - pt_found.vehicle_id(1:end-1);
                rho = dM./dx;

                % normalize to kc
                density.(linkStr).IC = rho/self.net.network_hwy.(linkStr).para_kc;
                density.(linkStr).X_grid_cum = pt_found.position;
